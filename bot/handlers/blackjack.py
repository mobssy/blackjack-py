...
def _get_game_keyboard(first_turn: bool = False):
    """
...
async def cmd_deal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_photo(
        ...
        caption=caption,
        reply_markup=_get_game_keyboard(first_turn=True),
    )
...
def _double_result(game: BlackjackGame) -> Tuple[GameOutcome, float]:
    """
    ...
    Returns:
        Tuple[GameOutcome, float]: (게임 결과, 정산 금액)
    """
    if is_bust(game.player_hand):
        outcome = GameOutcome.BUST
        return outcome, PayoutCalculator.calculate(outcome, game.bet)
    game.dealer_play()
...
async def cmd_double(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    outcome, payout = _double_result(game)
    await _finish_game(update, user_tg_id, game, outcome, payout)
